...
min_date, max_date = date(1950, 1, 1), date(2099, 12, 31)
...
with tab_sip:
    ...
    with col1:
        ...
        if st.button("Calculate Goal SIP"):
            try:
                m_rate = ((1 + t_rate/100)**(1/12)) - 1
                n = t_years * 12
                res = t_amt * (m_rate / (((1 + m_rate)**n - 1) * (1 + m_rate)))
    ...
    with col2:
        ...
        if st.button("Calculate Wealth"):
            try:
                m_rate = ((1 + w_rate/100)**(1/12)) - 1
                n = w_years * 12
                total = w_amt * (((1 + m_rate)**n - 1) / m_rate) * (1 + m_rate)
                invested = w_amt * n
...
with tab_swp:
    ...
    swp_withdraw = st.number_input("Withdrawal Per Month (₹)", key="swp_withdraw")
    ...
    if st.button("Calculate SWP"):
        try:
            ...
            st.markdown(f'<h2 class="result-text">Final Value: ₹ {round(balance, 2):,}</h2>', unsafe_allow_html=True)
            st.markdown(f'<span class="quote-text">{random.choice(all_quotes)}</span>', unsafe_allow_html=True)
        except: st.error("Check values")
    st.markdown('</div>', unsafe_allow_html=True)
...
with tab_lumpsum:
    st.markdown('<div class="input-card">', unsafe_allow_html=True)
    l_p, l_r, l_n = st.number_input("Investment (₹)", key="l_p"), st.number_input("Return (%)", key="l_r"), st.number_input("Years", key="l_n")
    if st.button("Calculate Lumpsum"):
        fv = l_p * ((1 + l_r/100) ** l_n)
        st.markdown(f'<p style="color: #E5E7EB;">Invested Amount: ₹ {round(l_p):,}</p>', unsafe_allow_html=True)
        st.markdown(f'<p style="color: #E5E7EB;">Estimated Returns: ₹ {round(fv - l_p):,}</p>', unsafe_allow_html=True)
...
with tab_ins:
    ...
    if st.session_state.ins_data:
        st.table(pd.DataFrame(st.session_state.ins_data))
        if st.button("Calculate Result"):
            try:
                res = xirr([pd.to_datetime(d['Date']) for d in st.session_state.ins_data], [d['Amount'] for d in st.session_state.ins_data]) * 100
        ...
        if st.button("Clear Data"): st.session_state.ins_data = []; st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)

# --- XIRR PRO ---
with tab_xirr:
    st.error("⚠️ INVESTMENT as NEGATIVE (-10000), RETURNS as POSITIVE (25000)")
...
with tab_rev_cagr:
    st.markdown('<div class="input-card">', unsafe_allow_html=True)
    r_i, r_c, r_y = st.number_input("Initial (₹)", key="r_i"), st.number_input("CAGR (%)", key="r_c"), st.number_input("Years", key="r_y")
    if st.button("Calculate Future Value"):
        res = r_i * ((1 + r_c/100) ** r_y)
